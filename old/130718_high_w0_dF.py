...
# Fit MEASURED peaks
#
x_data_expt, y_data_expt = numpy.loadtxt('Y24204K.txt', unpack=True, usecols = [0,1])
...
xMin2 = 13.0
xMax2 = 14.5
p2_guess = (2.22888,11.7171,1.14198)
...
# Fit CALCULATED data
#
x_data_theory, y_data_theory = numpy.loadtxt('Y24204L.txt', unpack=True, usecols = [0,1])
...
#----------------------------------------------------------------------------
#
# Perform two sum rules to test construction of sythesized curve 
#

# Sum rule 1: Oscillator Strength- calculate and compare n_eff 
# for sythesized and experimental curves

n_synth = [(x_synth*y_data_synth) for x_synth in x_data_synth]
n_eff_synth = numpy.zeros(len(x_data_synth))
for i,n_eff_integrand_synth in enumerate(n_synth):
	n_eff_synth[i] = integrate.simps(n_eff_integrand_synth,x_data_synth)

n_expt = [(x_expt*y_data_expt) for x_expt in x_data_expt]
n_eff_expt = numpy.zeros(len(x_data_expt))
for j,n_eff_integrand_expt in enumerate(n_expt):
	n_eff_expt[j] = integrate.simps(n_eff_integrand_expt,x_data_expt)

# Sum rule 2: Calculate and compare Index of refraction
#

# Apply sum rule to 2nd (after sythisised first peak) peak of calculated data 
xMin1 = 11.0
xMax1 = 13.0

y_fitData_synth=y_data_theory[(x_data_theory>xMin1)*(x_data_theory<xMax1)]
x_fitData_synth=x_data_theory[(x_data_theory>xMin1)*(x_data_theory<xMax1)]

i_synth = [((x_fit_synth*y_fitData_synth)**(-1.)) for x_fit_synth in x_fitData_synth]
index_synth = numpy.zeros(len(x_fitData_synth))
for k,index_integrand_synth in enumerate(i_synth):
	index_synth[k]= (1.-(2./numpy.pi)*integrate.simps(index_integrand_synth,x_fitData_synth))**(-1./2)

# Apply sum rule to 2nd peak of experimental data 
y_fitData_expt=y_data_expt[(x_data_expt>xMin1)*(x_data_expt<xMax1)]
x_fitData_expt=x_data_expt[(x_data_expt>xMin1)*(x_data_expt<xMax1)]

i_expt = [((x_fit_expt*y_fitData_expt)**(-1.)) for x_fit_expt in x_fitData_expt]
index_expt = numpy.zeros(len(x_fitData_expt))
for m,index_integrand_expt in enumerate(i_expt):
	index_expt[m]=(1.-(2./numpy.pi)*integrate.simps(index_integrand_expt,x_fitData_expt))**(-1./2)

#---------------------------------------------------------
#
# PLOT data, fits, and sythesised spectrum
#
...
# Plot the newly synthesised spectrum next to the original data for
# visual inspection...
pyplot.figure()
...
#---------------------------------------------------------
#
# PLOT sum rule tests for experimental and synthesised spectra
#

# Oscillator Strength Sum rule plot:
#

pyplot.figure()
pyplot.plot(x_data_synth,n_eff_synth, color='r', label='synthesised')
pyplot.plot(x_data_expt,n_eff_expt, color = 'k', label='experiment')

pyplot.title(r'Oscillator strength for synthesised and experimental data')
pyplot.xlabel(r'eV')
pyplot.ylabel(r'n$_{eff}$')

pyplot.legend(loc='upper left')
pyplot.show()

# Refractive index sum rule plot:
#

pyplot.figure()
pyplot.plot(x_fitData_synth,index_synth, color='r', label='synthesised')
pyplot.plot(x_fitData_expt,index_expt, color = 'k', label='experiment')

pyplot.title(r'Refractive Index for synthesised and experimental data')
pyplot.xlabel(r'eV')
pyplot.ylabel(r'n$_{refractive}$')
