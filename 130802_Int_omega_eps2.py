...
pp = PdfPages('plots/130802_Hopkins_pre_eV_sum_rule_integrand.pdf')
...
sum_rule    = empty(len(x_nopeak_eV))
sum_rule_042= empty(len(x_042_peak_eV))
sum_rule_062= empty(len(x_062_peak_eV))
sum_rule_082= empty(len(x_082_peak_eV))
sum_rule_102= empty(len(x_102_peak_eV))
sum_rule_132= empty(len(x_132_peak_eV))
sum_rule_182= empty(len(x_182_peak_eV))
...
pl.figure()
I_npk = empty(len(x_nopeak))
I_042 = empty(len(x_nopeak))
I_062 = empty(len(x_nopeak))
I_082 = empty(len(x_nopeak))
I_102 = empty(len(x_nopeak))
I_132 = empty(len(x_nopeak))
I_182 = empty(len(x_nopeak))
for i in range(len(x_nopeak)):
    I_npk[i]=(x_nopeak[i])*y_nopeak[i]
    sum_rule = trapz(I_npk,x_nopeak)
    
#eiz_nopeak[j] = 1. + (2./pi) * trapz(eiz_nopeak_arg,x_nopeak)

for k in range(len(x_042_peak)):
    I_042[k]=(x_042_peak[k])*y_042_peak[k]
    sum_rule_042 = trapz(I_042,x_042_peak)

for m in range(len(x_062_peak)):
    I_062[m]=(x_062_peak[m])*y_062_peak[m]
    sum_rule_062 = trapz(I_062,x_062_peak)    

for n in range(len(x_082_peak)):
    I_082[n]=(x_082_peak[n])*y_082_peak[n]
    sum_rule_082 = trapz(I_082,x_082_peak)    

for p in range(len(x_102_peak)):
    I_102[p]=(x_102_peak[p])*y_102_peak[p]
    sum_rule_102 = trapz(I_102,x_102_peak)    

for q in range(len(x_132_peak)):
    I_132[q]=(x_132_peak[q])*y_132_peak[q]
    sum_rule_132 = trapz(I_132,x_132_peak)    

for s in range(len(x_182_peak)):
    I_182[s]=(x_182_peak[s])*y_182_peak[s]
    sum_rule_182 = trapz(I_182,x_182_peak)    




pl.plot(  x_nopeak,I_npk, linestyle = '--')#label = r'$\rm{ab\,initio}$' )
pl.plot(x_042_peak,I_042, linestyle = '--')#label = r'$\omega_{0}= 0.64$')
pl.plot(x_062_peak,I_062, linestyle = '--')#label = r'$\omega_{0}= 0.94$')
pl.plot(x_082_peak,I_082, linestyle = '--')#label = r'$\omega_{0}= 1.25$')
pl.plot(x_102_peak,I_102, linestyle = '--')#label = r'$\omega_{0}= 1.55$')
pl.plot(x_132_peak,I_132, linestyle = '--')#label = r'$\omega_{0}= 2.01$')
pl.plot(x_182_peak,I_182, linestyle = '--')#label = r'$\omega_{0}= 2.76$')


#pl.plot(x_042_peak,sum_rule_042, linestyle = '--')#label = r'$\omega_{0}= 0.64$')
#pl.plot(x_062_peak,sum_rule_062, linestyle = '--')#label = r'$\omega_{0}= 0.94$')
#pl.plot(x_082_peak,sum_rule_082, linestyle = '--')#label = r'$\omega_{0}= 1.25$')
#pl.plot(x_102_peak,sum_rule_102, linestyle = '--')#label = r'$\omega_{0}= 1.55$')
#pl.plot(x_132_peak,sum_rule_132, linestyle = '--')#label = r'$\omega_{0}= 2.01$')
#pl.plot(x_182_peak,sum_rule_182, linestyle = '--')#label = r'$\omega_{0}= 2.76$')

pl.plot(    x_s,y_s,  label = r'$ab\,initio$')
pl.plot(x_042_s,y_042_s,  label = r'$\omega_0= 0.64$')
pl.plot(x_062_s,y_062_s,  label = r'$\omega_0= 0.94$')
pl.plot(x_082_s,y_082_s,  label = r'$\omega_0= 1.25$')
pl.plot(x_102_s,y_102_s,  label = r'$\omega_0= 1.55$')
pl.plot(x_132_s,y_132_s,  label = r'$\omega_0= 2.01$')
pl.plot(x_182_s,y_182_s,  label = r'$\omega_0= 2.76$')
